  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-40 bg-[#050505] border-b border-white/10">
      <div className="flex justify-between items-center max-w-[1400px] mx-auto px-6 py-4">
        <Link href="/" className="font-bold text-sm tracking-tight hover:opacity-70 transition-opacity uppercase select-none text-white">
          Humain Pulse
        </Link>
          ...
          {navItems.map((item) => (
            <Link
              ...
              className={`hover:text-emerald-400 transition-colors ${
                pathname === item.href || pathname?.startsWith(item.href + '/') ? 'text-white' : ''
              }`}
            >
              {item.label}
            </Link>
            ...
            {navItems.map((item) => (
              <Link
                ...
                className={`font-mono text-sm uppercase tracking-widest ${
                  pathname === item.href || pathname?.startsWith(item.href + '/') ? 'text-white' : 'text-zinc-400'
                }`}
              >
                {item.label}
              </Link>
